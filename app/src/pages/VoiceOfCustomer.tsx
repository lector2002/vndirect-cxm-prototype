    ...
    <div className="flex min-h-full min-w-[1100px] flex-col bg-slate-50/50">
      <header className="border-b border-border bg-white px-6 py-5">
        <div className="flex items-start justify-between gap-8">
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">Customer intelligence</p><h1 className="mt-1 text-2xl font-bold tracking-tight">Tiếng nói khách hàng</h1><p className="mt-1 text-xs text-muted-foreground">Từ phản hồi đa nguồn đến evidence và quyết định sản phẩm.</p></div>
      ...
      <div className="grid min-h-[610px] flex-1 grid-cols-[230px_minmax(390px,0.9fr)_minmax(390px,1.1fr)]">
        ...
        <aside className="bg-white">
          ...
          <div className="space-y-5 p-5">
            ...
            <section className="rounded-xl border border-primary/20 bg-primary/[0.04] p-4"><div className="flex items-center gap-2 text-[10px] font-semibold text-primary"><CircleAlert className="h-3.5 w-3.5" />Recommended action</div><p className="mt-2 text-[11px] font-medium leading-5 text-slate-800">{selected.nextAction}</p><div className="mt-3 flex items-center justify-between border-t border-primary/10 pt-3 text-[9px] text-muted-foreground"><span>Accountable owner</span><strong className="text-foreground">{selected.owner}</strong></div>{selected.touchpointId === 'tp-bond' && <Link to="/issues?issue=CXI-026" className="mt-3 inline-flex items-center gap-1 text-[10px] font-semibold text-primary hover:underline">Xem CX issue liên quan <ChevronRight className="h-3 w-3" /></Link>}</section>
            <p className="text-[9px] leading-4 text-muted-foreground">Đối chiếu sentiment với adoption, retention và complaint trước khi chốt roadmap. Click theme khác để drill xuống evidence.</p>
          </div>
        </aside>
      </div>
    </div>
