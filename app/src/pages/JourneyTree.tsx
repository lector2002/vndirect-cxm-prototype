...
export default function JourneyTree() {
  ...
  return <div className="relative min-h-full p-6">
    {selectedPhase && selectedFlow ? <PhaseJourneyBoard phase={selectedPhase} flow={selectedFlow} onBack={() => { setSelectedPhaseId(null); setSelectedFlowId(null); }} onSelectFlow={() => setFlowDrawerPhase(selectedPhase)} onSelectStage={setSelectedStage} /> : <PhaseOverview onSelect={setFlowDrawerPhase} phaseFilter={selectedCustomerPhaseId} />}
    ...
    {selectedStage && selectedPhase && <StageDrawer stage={selectedStage} phase={selectedPhase} onClose={() => setSelectedStage(null)} />}
  </div>;
}

function PhaseOverview({ onSelect, phaseFilter }: { onSelect: (phase: JourneyPhase) => void; phaseFilter: string }) {
...
function MetroStage({ stage, index, color, onClick }: { stage: JourneyStage; index: number; color: string; onClick: () => void }) {
  const signals = stageSignals(stage); const cov = coverageOf(signals); const volume = signals.reduce((sum, signal) => sum + signal.volumePerDay, 0);
  return <button onClick={onClick} className="group relative text-left"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border-4 border-[hsl(222,45%,8%)] text-sm font-bold transition-transform group-hover:scale-110" style={{ background: color, color: '#08111e' }}>{index}</span><div className="mt-4 rounded-xl border border-border bg-card p-4 transition-colors group-hover:border-primary/60"><div className="flex items-start justify-between gap-2"><h3 className="text-sm font-semibold leading-snug">{stage.name}</h3><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground group-hover:text-primary" /></div><p className="mt-2 line-clamp-2 text-[11px] leading-5 text-muted-foreground">{stage.description}</p><div className="mt-4 flex items-end justify-between"><div><div className="text-[10px] text-muted-foreground">Volume / ngày</div><div className="mt-0.5 text-sm font-bold">{volume ? fmtNum(volume) : '—'}</div></div><div className="text-right"><div className="text-[10px] text-muted-foreground">Coverage</div><div className="mt-0.5 text-sm font-bold" style={{ color }}>{cov.score}%</div></div></div><div className="mt-3"><CoverageBar stats={cov} height="h-1.5" /></div><div className="mt-2 text-[10px] text-muted-foreground">{signals.length} signal · {cov.gap} gap</div></div></button>;
}

function StageDrawer({ stage, phase, onClose }: { stage: JourneyStage; phase: JourneyPhase; onClose: () => void }) {
