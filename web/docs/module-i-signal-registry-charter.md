...
| F6 | Đếm cộng lên | Mọi con số tổng **kèm mẫu số**; flow chưa chép bước **không nằm trong mẫu số** của bất kỳ tỉ lệ nào — test: thêm một flow không bước thì mọi tỉ lệ **không đổi**, chỉ ô "chưa đánh giá được" tăng 1. **Thêm vào BẢN SAO trong test, KHÔNG thêm vào fixture đang ship** — đụng fixture thật là mở một mặt trận không cần thiết. ⚠️ **Công thức dựng đã đổi sau I2:** `Flow.verified`/`Flow.observed` **không còn tồn tại** và **luật 13/14 đã khuyết** — flow thêm vào chỉ cần `steps: []`, hai trục nay suy tại chỗ đọc bằng `flowHasSourceCitation()` / `flowStepsCopied()`. Ai còn set `observed`/`verified` là đang viết theo bản charter cũ |
