...
| | Việc |
|---|---|
| ~~**C1**~~ | **CHỐT 12/08 — owner chọn GIỮ 3 CỘT.** `AtlasStepInspector` không dựng lại ô "Evidence coverage" ở dạng trống. Lý do đứng được: một ô trống chỉ nói ra được nghĩa của nó bằng **một câu giải thích**, mà luật giao diện 11/08 cấm đúng câu đó. Code đã đúng sẵn (`AtlasStepInspector.tsx` grid-cols-3) — chốt này **không sửa dòng nào**, chỉ đóng lại một cửa |
...
| ~~**C6**~~ | **ĐÓNG 12/08 — MOOT, không còn việc để làm.** Nội dung cũ: luồn `data.sources` + `data.asOf` qua `AtlasStepInspector` để tab "Chỉ số liên kết" hiện lại dòng độ tươi. **Tiền đề đã hỏng giữa lúc chờ:** chuỗi thay thế `metricFreshnessText()` **bị xoá 11/08** (`domain/sources.ts:144`) vì chính nó là thứ luật giao diện 11/08 cấm — ba đoạn luận giải *"trễ 4 giờ kể từ lần giao cuối · đã giao đủ đến 27/07/2026 · đang nhận"*. Luồn prop xong thì **không còn gì để gọi**. Câu *"ở `#/rules` thì đã hiện đầy đủ"* trong bản C6 gốc cũng **hết đúng** từ 11/08: bảng chỉ số ở đó nay không khai gì về độ tươi. **Trạng thái sau khi đóng: không chỗ nào trong app khai độ tươi ở MỨC CHỈ SỐ** — Atlas khớp `#/rules`, không còn hai giọng. Độ tươi thật của từng **nguồn** vẫn ở `#/sources` (`lagText()` + badge hạng). Muốn mở lại thì phải quyết trước *hiện DỮ LIỆU gì* (ví dụ badge hạng + mốc giao cuối của nguồn xấu nhất qua `worstSource()`, đơn vị NGÀY theo T-1), **không phải** dựng lại câu văn |
...
|---|---|---|
| F1 | Bảng liệt kê đủ điểm đo | Số dòng bảng **bằng** `data.signals.length`, không phụ thuộc bước/flow nào đang chọn. **Bổ sung 12/08 (redesign, owner chốt):** màn có bộ lọc — ba dòng đầu khối ① bấm được, cộng một ô tìm — và bộ lọc đó **TÔ, KHÔNG CẮT**: nhóm khớp đẩy lên đầu và giữ độ đậm, phần còn lại mờ đi nhưng **vẫn còn đủ dòng**. Owner chọn cơ chế này chứ không phải ẩn dòng vì kiểm kê toàn hệ mà mẫu số rời khỏi màn thì không còn là kiểm kê. Test ghim `signals.length` dòng ở **mọi** trạng thái lọc, và ghim số trên chip = số dòng được tô (hai đường đếm khác nhau: chip đếm bằng hàm `domain/`, tô đếm bằng vị từ `features/signals/facets.ts`). **Sửa 12/08 chiều (owner chốt, thay vế "đẩy lên đầu" ở trên):** bảng **chia nhóm theo phase** và dòng không khớp mờ **TẠI CHỖ** trong nhóm của nó — lý do là quy mô, tên event sẽ lên tới vài trăm và ở quy mô đó "đẩy lên đầu" làm bảng nhảy chỗ mỗi lần lọc. Gom nhóm là **phân hoạch**, không phải lọc: test ghim tổng số dòng mọi nhóm = `data.signals.length`, ghim mỗi điểm đo nằm đúng một nhóm, và ghim số "khớp/tổng" ở tiêu đề mỗi nhóm = số dòng được tô **trong chính nhóm đó**. Điểm đo có chuỗi allocate đứt đi vào một nhóm riêng đứng cuối, **không** bị nhét vào một phase có thật (F2) và **không** rơi khỏi bảng. Hàng tiêu đề nhóm **không mang** `data-testid="signal-row-*"` — testid đó là mẫu số của chính F1. **Bổ sung 12/08 tối (owner chốt — sửa vế "số dòng bằng data.signals.length" ở đầu ô này):** mỗi nhóm **THU GỌN được**, nên số dòng RENDER không còn luôn bằng tổng. Thu gọn hợp lệ nhờ ĐÚNG MỘT điều kiện, và điều kiện đó lấy thẳng từ lý do cấm ẩn dòng ghi ở trên — *mẫu số rời khỏi màn thì không còn là kiểm kê*: **tiêu đề nhóm ở lại và còn nguyên số đếm của nó**. Nghiệm thu đổi thành: Σ(dòng render của nhóm đang mở) + Σ(số ở tiêu đề nhóm đang thu gọn) = `data.signals.length`, ở MỌI tổ hợp lọc × thu gọn. Ba ràng buộc kèm: mặc định **mở hết**; thu gọn **không đổi một con số đếm nào** (chip ①, đếm thanh công cụ, "khớp/tổng" từng nhóm); hồ sơ đi tới/lui vẫn chạy hết `data.signals` **kể cả điểm đo trong nhóm đang thu gọn** — thu gọn là việc của mắt, không phải một phép lọc dữ liệu |
...
| **I4b** | **Hồ sơ một điểm đo — bốn mặt**. **Sửa 18/08 (owner, redesign phương án A):** bấm dòng nay mở DRAWER tóm tắt đứng CẠNH bảng (`SignalDrawer.tsx`), hồ sơ đầy đủ bốn mặt lùi một nấc sau nút "Open full profile"; đóng hồ sơ quay về bảng + drawer đang mở, không về bảng trần. **Bổ sung 19/08 (ADR-004):** drawer thêm hai hàng — *Evaluation* (trạng thái theo ngưỡng riêng `cfg.signal`, cùng `signalEval` với nhóm mới bên `#/rules`, unknown nói lý do chứ không rơi về ok) và *Threshold* (band chỉ-đọc + lối `#/rules/signal`); điểm đo chưa đặt ngưỡng nói "not set" | F2 · F4 · D5 · D6 | I4a. **F3 phần độ tươi nguồn HOÃN** — chặn bởi A1 |
