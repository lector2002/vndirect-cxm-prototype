...
describe("SrcMatrix", () => {
  it("số hàng = sources.length", () => {
    render(<SrcMatrix sources={seed.sources} metrics={seed.metrics} cfg={cfgDefault} asOf={seed.asOf} />);
  ...
  it("số dấu không-phải-● ở cột Trạng thái khớp số nguồn health !== 'ok'", () => {
    ...
    const expectedNotOk = seed.sources.filter((s) => sourceHealth(s, cfgDefault, seed.asOf) !== "ok");
  ...
  it("compact=true ẩn cột metric, giữ cột Trạng thái", () => {
    render(<SrcMatrix sources={seed.sources} metrics={seed.metrics} cfg={cfgDefault} asOf={seed.asOf} compact />);
